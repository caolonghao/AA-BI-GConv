...
from .edge_aware_gcn import GRU_EAGCN, AG_EAGCN
...
class SEG_Module(nn.Module):
    def __init__(self, channel, postgnn = "APPNP"):
        ...
        self.GCN = AG_EAGCN(num_in=2, plane_mid=1, mids=32, depth=8, postgnn=postgnn, alpha=0.3)
    ...
    def forward(self, x1, x2, x3, edge):
        ...
        seg_gcn, graph_regulation_loss = self.GCN(seg, edge)
        seg_gcn = seg_gcn + seg

        seg = self.upsample(seg_gcn)

        return seg, graph_regulation_loss
...
class ODOC_seg_edge(nn.Module):
    ...
    def forward(self, x):
        ...
        edge = F.interpolate(
            edge_feat, size=(32, 32), mode="bilinear", align_corners=True
        )

        seg, graph_regulation_loss = self.seg_layer(x4_rfb, x3_rfb, x2_rfb, edge)

        seg_output = torch.sigmoid(seg)

        return seg_output, torch.sigmoid(edge_feat), graph_regulation_loss
