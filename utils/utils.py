...
def model_test(model, test_loader):
    ...
    with torch.no_grad():
        ...
        print("95CI_cup_dice, lower: {:.4f}, higher: {:.4f}".format(confidence_lower, confidence_upper))
        ...
        confidence_lower = sorted_scores[int(0.025 * len(sorted_scores))]
        confidence_upper = sorted_scores[int(0.975 * len(sorted_scores))]
        print("95CI_disc_dice, lower: {:.4f}, higher: {:.4f}".format(confidence_lower, confidence_upper))
        # BC_cup_CI
        sorted_scores = np.sort(np.array(CI_cup_BC))
        confidence_lower = sorted_scores[int(0.025 * len(sorted_scores))]
        confidence_upper = sorted_scores[int(0.975 * len(sorted_scores))]
        print("95CI_cup_BC, lower: {:.4f}, higher: {:.4f}".format(confidence_lower, confidence_upper))

        # BC_disc_CI
        sorted_scores = np.sort(np.array(CI_disc_BC))
        confidence_lower = sorted_scores[int(0.025 * len(sorted_scores))]
        confidence_upper = sorted_scores[int(0.975 * len(sorted_scores))]
        print("95CI_disc_BC, lower: {:.4f}, higher: {:.4f}".format(confidence_lower, confidence_upper))
